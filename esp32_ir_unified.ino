...
void validatePins() {
  ...
  for (auto& c : chk) {
    ...
    if (w.length()) { wsSerial.printf("[Pins] WARNING %s GPIO%d: %s\n", c.n, c.p, w.c_str()); ok=false; }
  }
  if (ok) wsSerial.println("[Pins] All GPIO assignments OK");
}
  ...
  server.on("/api/dazzler/start", HTTP_POST, [](AsyncWebServerRequest* req) {},
    nullptr, [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t) {
      ...
      if (!deserializeJson(doc, data, len)) {
        ...
        dz.stopAt    = secs > 0 ? millis()+secs*1000 : 0;
      }
...
void loop() {
  ...
  if (isLearning) {
    if (millis() > learnTimeout) {
      ...
      if (irResults.value != REPEAT && irResults.decode_type != UNKNOWN)
